...
func (client *LightsparkClient) FundNode(nodeId string, amountSats int64) (
	*objects.CurrencyAmount, error,
	...
	response, err := client.ExecuteGraphql(scripts.FUND_NODE_MUTATION, variables, nil)
	if err != nil {
		return nil, err
	}
...
func (client *LightsparkClient) PayInvoice(nodeId string, encodedInvoice string,
	...
	return client.PayInvoiceWithIdempotencyKey(
		nodeId,
		encodedInvoice,
		timeoutSecs,
		maximumFeesMsats,
...
// PayInvoiceWithIdempotencyKey sends a payment to a node on the Lightning Network, based on the invoice
// (as defined by the BOLT11 specification) that you provide.
// If you are in test mode, the invoice has to be generated by create_test_mode_invoice mutation.
//
// Args:
...
func (client *LightsparkClient) FetchOutgoingPaymentsByPaymentHash(
	...
	response, err := client.ExecuteGraphql(scripts.OUTGOING_PAYMENTS_FOR_PAYMENT_HASH_QUERY, variables, nil)
	if err != nil {
		return nil, err
	}
...
func (client *LightsparkClient) FetchOutgoingPaymentByIdempotencyKey(idempotencyKey string) (*objects.OutgoingPayment, error) {
	...
	response, err := client.ExecuteGraphql(scripts.OUTGOING_PAYMENT_FOR_IDEMPOTENCY_KEY_QUERY, variables, nil)
	if err != nil {
		return nil, err
	}
...
func (client *LightsparkClient) FetchInvoiceByPaymentHash(paymentHash string) (*objects.Invoice, error) {
	...
	response, err := client.ExecuteGraphql(scripts.INVOICE_FOR_PAYMENT_HASH_QUERY, variables, nil)
	if err != nil {
		return nil, err
	}
